...
def main():
    if len(sys.argv) < 2:
        ...
        print("Available commands: create, format, refresh, help, netspeed, sound, video, downloadvs, requirement, power, sf, sif, up, run, psg, hs, sr, wea, pr")
        return
...
Available commands:
  ...
  pr <sessions>         Pomodoro timer with customizable session count (runs in background)
  pr stop              Stop running Pomodoro timer
  pr st                Show Pomodoro timer status and recent activity
""",
            "fr": """
...
Commandes disponibles :
  ...
  pr <sessions>         Minuteur Pomodoro avec nombre de sessions personnalisable (s'exécute en arrière-plan)
  pr stop              Arrêter le minuteur Pomodoro en cours
  pr st                Afficher le statut du minuteur Pomodoro et l'activité récente
""",
            "ar": """
...
الأوامر المتاحة:
  ...
  pr <الجلسات>              مؤقت بومودورو مع عدد جلسات قابل للتخصيص (يعمل في الخلفية)
  pr stop                 إيقاف مؤقت بومودورو قيد التشغيل
  pr st                   عرض حالة مؤقت بومودورو والنشاط الأخير
""",
            "de": """
...
Verfügbare Befehle:
  ...
  pr <Sitzungen>          Pomodoro-Timer mit anpassbarer Sitzungsanzahl (läuft im Hintergrund)
  pr stop                 Laufenden Pomodoro-Timer stoppen
  pr st                   Pomodoro-Timer Status und letzte Aktivität anzeigen
"""
        }
...
"""
        ...
        if subc not in avup:
            ...
            print("availbe commands: st: download latest AllTool stable version, pv: download latest AllTool preview version.")
        elif subc == "st":
            ...
            def update_script():
                ...
                if remote_version and (local_version is None or version.parse(remote_version) > version.parse(local_version)):
                    with open(LOCAL_PATH, "w", encoding="utf-8") as f:
                        f.write(remote_code)
                    print(f"✅ Updated AllTool.py from {local_version} to {remote_version}")
                else:
                    print("✅ Already up to date.")
            update_script()
        elif subc == "pv":
            githubst = "still working"
