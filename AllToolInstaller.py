...
def install_system_packages(distribution, selected_packages=None):
    ...
    package_list = selected_packages if selected_packages else packages[distribution]
    ...
    for i, pkg in enumerate(package_list, start=1):
        ...
        run_command(cmd, f"Installing {pkg}")
...
def install_python_packages(selected_packages=None):
    python_packages = selected_packages if selected_packages else ["requests", "beautifulsoup4"]
    ...
    print("\n✅ All selected Python packages installed!")
...
def main():
    ...
    if choice == '2':
        ...
        print("\nSelect system packages to install (comma separated numbers):")
        for i, pkg in enumerate(all_system_packages, start=1):
            print(f"{i}) {pkg}")
        selection = input("Packages to install (e.g., 1,3,5): ").strip()
        ...
        print("\nSelect Python packages to install (comma separated numbers):")
        for i, pkg in enumerate(python_pkg_list, start=1):
            print(f"{i}) {pkg}")
        selection_py = input("Python packages to install (e.g., 1,2): ").strip()
    ...
    print("Run : source ~/.bashrc (or ~/.zshrc) to update your PATH if needed.")
    delete_installer()
